...
export default function Testimonial({ imgName, children, className, ...rest }) {
  const allClasses = classnames("testimonial", className);

  const imgLink = <img src={`./sources/${imgName}`} alt={imgName} />;

  return (
    <div className={allClasses} {...rest}>
      <div className="testimonial-img">{imgLink}</div>
      <div>
        <svg
